...
// Helper to get section order with fallback
const getSectionOrder = (resumeData: ResumeData) => {
  return resumeData.sectionOrder || ["summary", "experience", "projects", "education", "skills"]
}
...
const TechnicalTemplate = ({ resumeData }: { resumeData: ResumeData }) => {
  ...
  // Filter out skills, as it's in the sidebar
  const mainOrder = order.filter(key => key !== 'skills')
    ...
    <div className="flex h-full flex-col bg-white p-8 text-black gap-3">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          {mainOrder.map((key) => leftSections[key])}
        </div>
